"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ErpCashHandoversService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
const cash_handover_entity_1 = require("./entities/cash-handover.entity");
const sequence_service_1 = require("./sequence.service");
const cash_handover_status_enum_1 = require("./enums/cash-handover-status.enum");
function dec4(n) {
    return n.toFixed(4);
}
let ErpCashHandoversService = class ErpCashHandoversService {
    constructor(repo, sequences) {
        this.repo = repo;
        this.sequences = sequences;
    }
    findAll() {
        return this.repo.find({
            order: { id: "DESC" },
            relations: { handedBy: true, receivedBy: true },
        });
    }
    async findOne(id) {
        const row = await this.repo.findOne({
            where: { id },
            relations: { handedBy: true, receivedBy: true },
        });
        if (!row)
            throw new common_1.NotFoundException(`Cash handover ${id} not found`);
        return row;
    }
    async create(dto) {
        const handoverNumber = await this.sequences.next("CH");
        const entity = this.repo.create({
            handoverNumber,
            fromType: dto.fromType,
            fromId: dto.fromId,
            toType: dto.toType,
            toId: dto.toId,
            amount: dec4(dto.amount),
            originalCurrency: dto.originalCurrency ?? "USD",
            originalAmount: dto.originalAmount !== undefined ? dec4(dto.originalAmount) : null,
            handoverDate: new Date(dto.handoverDate),
            handedBy: { id: dto.handedById },
            notes: dto.notes ?? null,
            status: cash_handover_status_enum_1.CashHandoverStatus.PENDING,
            confirmedAt: null,
        });
        return this.repo.save(entity);
    }
    async update(id, dto) {
        const row = await this.findOne(id);
        if (dto.status !== undefined)
            row.status = dto.status;
        if (dto.receivedById !== undefined) {
            row.receivedBy = dto.receivedById
                ? { id: dto.receivedById }
                : null;
        }
        if (dto.confirmedAt !== undefined)
            row.confirmedAt = dto.confirmedAt ? new Date(dto.confirmedAt) : null;
        if (dto.notes !== undefined)
            row.notes = dto.notes ?? null;
        return this.repo.save(row);
    }
};
exports.ErpCashHandoversService = ErpCashHandoversService;
exports.ErpCashHandoversService = ErpCashHandoversService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(cash_handover_entity_1.CashHandover)),
    __metadata("design:paramtypes", [typeorm_2.Repository,
        sequence_service_1.SequenceService])
], ErpCashHandoversService);
//# sourceMappingURL=cash-handovers.service.js.map